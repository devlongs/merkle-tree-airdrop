...
function main() {
  ...
  const filename = "merkle_proof/csv/token_claimers.csv";

  // what file should we write the merkel proofs too?
  const output_file = "merkle_proof/outputs_file/permitted_claim.json";

  //file that has the user claim list
  const userclaimFile = "merkle_proof/outputs_file/token_claimlist.json";
  ...
  function write_leaves(merkle_tree, user_dist_list, token_dist, root) {
    ...
    for (line = 0; line < user_dist_list.length; line++) {
      ...
      // create row["itemID"],["itemID"],ist object
      const user_dist = {
        leaf: leaf,
